...
lines!( ax, map(x->x^2,x), x, linewidth=3, color=map(x->x> 0 ? :darkblue : :lightblue,x))
hlines!( ax, [-3], linewidth=3, color=:darkblue)
xlims!(-3,3)
...
streamplot!( ax, x-> fixed_point(x,[0,-1]),
...
streamplot!( ax, x-> fixed_point(x,[0,-1]) + 0.2*fixed_point(x,[0,1/2]),
    -2..2, -2..2, density=0.5, linewidth=3, colormap=[:lightblue], stepsize=0.05)
scatter!( ax, [0], [-1], markersize=8, color=:darkblue)
scatter!( ax, [0], [1/2], markersize=8, color=:gold)
