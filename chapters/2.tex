...
\caption{Adopting Waddington's epigenetic landscapes metaphor for representing the set of available behaviours for two phenotypes in response to a controlled condition $p$. Phenotypes are related by changes in landscape topology via changes in $\theta$}
...
\begin{Figure}
	\includegraphics[scale=0.55]{local-bifurcations} 
...
\begin{Figure}
	...
	\caption{The \emph{saddle-node} bifurcation creates or annihilates stable (dark blue) and unstable (light blue) pairs of fixed points. A critical slow-down that scales like $t\sim\frac{1}{\sqrt{|p-p^\star|}}$ can be used to detect its onset.}
	\label{fig:saddle-node}
\end{Figure}

\begin{Figure}
	\includegraphics[scale=0.6]{hopf}
	\caption{In a \emph{Hopf} bifurcation a stable (dark blue) fixed point becomes an unstable (light blue) focus that pushes trajectories towards a limit cycle (dark blue region). The limit cycle amplitude grows as $\sqrt{p-p^\star}$.}
	\label{fig:hopf}
\end{Figure}

\begin{Figure}
	\includegraphics[scale=0.6]{pitchfork}
	\caption{In \emph{Pitchfork} bifurcations a single stable (dark blue) fixed point splits into two stable fixed points separated by a saddle point (light blue). The distance between points grows as $\sqrt{p-p^\star}$. Small perturbations in $p$ breaks the pitchfork into a \emph{saddle node} bifurcation and a stable branch of fixed points.}
	\label{fig:pitchfork}
\end{Figure}

\begin{Figure}
	\includegraphics[scale=0.6]{transcritical}
	\caption{In \emph{transcritical} bifurcations a stable (dark blue) and unstable (light blue) fixed point collide and exchange stability. The distance between points grows as $p-p^\star$. Perturbations in $p$ breaks the transcritical into two \emph{saddle-node} bifurcations.}
	\label{fig:transcritical}
\end{Figure}

\begin{Figure}
	\includegraphics[scale=0.6]{cyclic-fold}
	\caption{The \emph{cyclic fold} bifurcation is a \emph{global} bifurcation that creates or annihilates stable (dark blue region) and unstable (light blue region) pairs of limit cycles.}
	\label{fig:cyclic-fold}
\end{Figure}

\begin{Figure}
	\includegraphics[scale=0.6]{homoclinic}
	\caption{A \emph{homoclinic} bifurcation occurs when a saddle point (light blue) collides with a limit cycle (dark blue region) forming a \emph{homoclinic} orbit just before breaking the cycle. It is accompanied by a period slow-down that scales like $t\sim|\log|p-p^\star||$.}
	\label{fig:homoclinic}
\end{Figure}

\begin{Figure}
	\includegraphics[scale=0.6]{infinite-period}
	\caption{The \emph{infinite period} bifurcation is a \emph{global} bifurcation where a \emph{saddle-node} bifurcation occurs along the limit cycle (dark blue region), breaking it. It is accompanied by a period slow-down that scales like $t\sim\frac{1}{\sqrt{|p-p^\star|}}$.}
...
The eigenvalue distribution $P(\lambda)$ of the diffusive term can be derived by applying the Circular Diagonalization Theorem \cite{Ingleton1956TheMatrices} to the $K\times K$ matrix representing its second-order central difference approximation with appropriate boundary conditions on $K$ sites, and then taking the limit as $K\rightarrow\infty$. For example, the diffusive term on a one dimensional ring can be represented as
\begin{align}
...
\end{align}
whose eigenvalues are always negative $\lambda \le 0$ and given by the arcsine distribution (derivation in Appendix \ref{appendix:eigenvalue-distributions}). In the absence of reaction terms, diffusion drives the dynamics towards a homogeneous steady state $\steady$ given be the eigenvector $\eigenvector$ corresponding the vanishing eigenvalue $\lambda=0$. We can Taylor expand the reaction term in equation \eqref{eq:reaction-diffusion} according to equation \eqref{eq:linearised-differential-equations} to investigate the geometry of state space in the vicinity of the homogeneous state $\steady$ yielding an equation linear in perturbations $\delta u$
\begin{equation}
\frac{d}{dt}(\delta u) = \mathcal{L}\,\delta u + \mathcal{O}(\delta u^2)
\where \mathcal{L} := 
	\left.\jacobian\right|_{u=\steady}\!\! + D\frac{\partial^2 }{\partial x^2}
	\label{eq:linearised-reaction-diffusion}
\end{equation}
The linear operator $\mathcal{L}$ has an eigenvalue distribution that determines whether perturbations $\delta u$ shirk to zero or appear to grow. The matrix representation of this operator can be written in terms of kronecker products
\begin{equation}
    \mathcal{L} \rightarrow \left.\jacobian\right|_{u=\steady}\!\!\!\!\!\!\!\!\!\!\!\otimes\mathbb{1}_K + D\otimes\frac{\partial^2}{\partial x^2}
\end{equation}
where the product between the $N \times N$ Jacobian and the $K \times K$ identity matrix $\mathbb{1}_K$ yields an $NK \times NK$ matrix representing the reactive terms on for each component on each site. The product between the diagonal matrix $D$ and the matrix representation of the diffusive term also yields an $NK \times NK$ matrix that couples neighbouring sites with with magnitudes corresponding to the mobility of each component. Figure \ref{fig:reaction-diffusion-spectrum} illustrates how the eigenvalue distributions of $\mathcal{L}$ appear as distorted and transformed arcsine distributions. The transformation is non-trivial and depends on eigenvalues of the Jacobian as well as the diffusion coefficients.
\begin{Figure}
\includegraphics[width=0.8\linewidth]{laplacian-spectrum}
\caption{Eigenvalue distributions $P(\lambda)$ for the linearised reaction-diffusion operator $\mathcal{L}$ of an activator-inhibitor $N=2$ system on a one dimensional ring showing three cases: diffusion only (gray), reaction with equal diffusion coefficients (dark blue) and with differential diffusion (light blue) where the distribution crosses zero, destabilising the homogeneous steady state $\steady$}
\label{fig:reaction-diffusion-spectrum}
\end{Figure}

\subsubsection{Turing Bifurcations}
We have seen how reaction and diffusion terms can come together and cause the eigenvalue distribution to cross zero (Figure \ref{fig:reaction-diffusion-spectrum}) de-stabilising the homogeneous steady state $\steady$. This may lead to pattern formation, similar to how de-stabilising a focus may lead to limit cycles in the Hopf bifurcation. This is known as a \emph{Turing bifurcation} and can also be analysed from the perspective of spatial modes $k$.
...
\begin{equation}
	\left(\left.\jacobian\right|_{u=\steady} - |k|^2D - \omega I \right)\mathcal{F}[u] = 0
\end{equation}
identifying $\omega$ and the eigenvalues to corresponding eigenvectors $\mathcal{F}[u]$. Using the determinant equation
\begin{equation}
	\left|\,\,\left.\jacobian\right|_{u=\steady} - |k|^2D - \omega I \right| = 0
...
\end{equation}
which can be used as a scalar potential that generates a basis function field $F(u)$ that drives the dynamics towards $\partial\Omega$. Once a trajectory reaches the limit set we want it to remain there and therefore we would like the basis function to vanish on the limit set
\begin{equation}
    F(u\in\partial\Omega)=0
    \label{eq:steady-state-condition}
\end{equation}
Since we are trying to construct basis functions, another useful property to have is locality. Trajectories far away from a limit set should not feel its affect will therefore be attracted by the limit sets closest to it. Locality can be expressed as the basis function vanishing far away from its limit set

\begin{equation}
    N(u,\partial\Omega)\rightarrow \infty \implies F(u)\rightarrow 0 
    \label{eq:locality-condition}
\end{equation}
A basis function that uses \eqref{eq:distance-function} as a scalar potential, satisfies steady state \eqref{eq:steady-state-condition} and locality \eqref{eq:locality-condition} conditions can be constructed as
\begin{equation}
	F(u) = -N(u,\partial\Omega)\mathbb{e}^{-N(u,\partial\Omega)}\frac{\partial}{\partial u}N(u,\partial\Omega)
	\label{eq:scalar-potential}
\end{equation}
If the limit set $\partial\Omega$ is a connected surface we can additionally specify an angular momentum field $\omega(u)$ that gives rise to dynamics tangent to the limit set, which leading to limit cycles. The trajectory is not allowed to escape the limit set while it is circulating and therefore the basis function must obey a tangent condition
\begin{equation}
\left.\frac{\partial}{\partial u}N(u,\partial\Omega)\cdot F(u)\right|_{u\in\partial\Omega}=0
    \label{eq:tangent-condition}
\end{equation}
For a given basis function we can now decompose the field into tangential components responsible for cycles and normal components that attract trajectories towards the limit set. Before we can combine the two components together we need to decide what the relative weighting coefficients should be for each component. While both components should still obey locality \eqref{eq:locality-condition}, the relative magnitudes of each component as we move away from a limit set will determine the direction of the field. If we allow the tangential components to vanish faster than the normal components, then the field will point normally towards the limit set. The result is that all limit sets look like fixed points at a sufficiently large distance and we believe this yields to most simple field $F$ for a given limit set $\partial\Omega$. The basis function
\begin{equation}
...
\end{equation}
is a choice that satisfies all our requirements. We note that the cross product is only defined for three dimensions $N=3$; the higher dimensional analogues of this operation is the wedge product $\wedge$ defined in the language of exterior algebra \cite{Flystad2015TheMathematics}. We can now create a superposition of basis functions with amplitudes $\alpha_{k}$ to construct a field that has multiple limit sets $\partial\Omega_k$ with their associated angular momenta $\omega_k(u)$
\begin{equation}
...
\end{equation}
The amplitudes $\alpha_{k}$ determine the relative sizes of the basins of attraction for each limit set. In addition we can let $\alpha_{k}<0$ to specify unstable structures.
...
\includegraphics[width=80mm]{figures/morph2.png}
\caption{Top: \textit{Hox} gene expression patterns in body segments of \emph{Drosophila}. Bottom: Mutation where legs grow in-place of antenna. Figures from \cite{Reece2014CampbellBiology,Tzou2022HoxGene}.}
\label{fig:morph}
\end{Figure}
...
\end{Figure}
When a model $\rates(u,p)$ is absent it becomes difficult to know which observations to collect. In the era of high-throughput biology, the solution to this is to simply collect as many observations as possible and search the high-dimensional data for mechanisms that elucidate what the organism model could be. Such data may include flow cytometry, proteomics (MS/MS), transcriptomics (scRNA-seq) metabolomics (MS) and a wide variety of next-generation sequencing data \cite{Perkel2021Single-cellAge}. In the context of this thesis, problem of differentiating phenotypes becomes a matter of extracting bifurcation information from high dimensional \emph{universal function approximators}.
...
This approach may break down if multi-modal distributions exist in the data. This would be the case if something happened to prevent a sub-population of cells from switching from one state another other. Reasons for this could include too much cell burden or not enough time given for cells to reach a steady state. In this case, quantifying the efficiency of switching from either side of the cusp could be a quantity of interest. In Chapter \ref{chapter:double-exclusive} we opted for a more basic method for extracting the location of a cusp in an effort not to introduce additional parameters such as $\epsilon$ that may have to be tuned. In the afterword (Section \ref{afterword:inference}) we discuss the benefits of a more data-driven approach, as shown in Figure \ref{fig:cusp-sampling}, for extracting the cusp location.
