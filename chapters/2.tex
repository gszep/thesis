...
\begin{Figure}
	...
	\caption{In a \emph{Hopf} bifurcation a stable (dark blue) fixed point becomes an unstable (light blue) focus that pushes trajectories towards a limit cycle (shaded dark blue). The limit cycle amplitude grows as $\sqrt{p-p^\star}$.}
	\label{fig:hopf}
\end{Figure}
...
\begin{Figure}
	...
	\caption{The \emph{cyclic fold} bifurcation is a \emph{global} bifurcation that creates or annihilates stable (dark blue) and unstable (light blue) pairs of limit cycles.}
	\label{fig:cyclic-fold}
\end{Figure}

\begin{Figure}
	\includegraphics[scale=0.6]{homoclinic}
	\caption{A \emph{homoclinic} bifurcation occurs when a saddle point (light blue) collides with a limit cycle (dark blue) forming a \emph{homoclinic} orbit just before breaking the cycle. It is accompanied by a period slow-down that scales like $t\sim|\log|p-p^\star||$.}
	\label{fig:homoclinic}
\end{Figure}

\begin{Figure}
	\includegraphics[scale=0.6]{infinite-period}
	\caption{The \emph{infinite period} bifurcation is a \emph{global} bifurcation where a \emph{saddle-node} bifurcation occurs along the limit cycle (dark blue), breaking it. It is accompanied by a period slow-down that scales like $t\sim\frac{1}{\sqrt{|p-p^\star|}}$.}
