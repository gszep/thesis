...
\end{align}
Here we kept the first-order derivative because we would still like to see how the dynamics scales with time as we approach to degeneracy. This is a Bernoulli differential equation \cite{Hairer1993SolvingI} with constant coefficients and has a general solution
\begin{align}\!\!\!\!\!
	T_{\lambda}(t) = \left(
...
dimensional cases and thus its eigenvalues is simply the sum one dimensional dispersions \cite{Laub2004MatrixEngineers}
...
\begin{align*}
  ...
  \frac{\mathbb{e}^{\Lambda_L\mathbb{i}k}\exp[\sum_{i}u_i\mathbb{i}k]}
  {\sqrt{\prod_{i}(1-u_i^2/4) }}
  \,\mathrm{d}u\mathrm{d}k\\
  &=\frac{1}{2\pi Z}
  \int_{-\infty}^{\infty}\mathbb{e}^{\Lambda_L\mathbb{i}k}
  \prod_{i}\int_{-2}^{2}\!
  \frac{\mathbb{e}^{u_i\mathbb{i}k}}
  {\sqrt{1-u_i^2/4}}
  \,\mathrm{d}u_i\mathrm{d}k
\end{align*}
The fourier representation of the delta function allowed the
factorisation of the integral. We recognise a repeated Bessel integral and replace it with the Bessel function of the first kind $J_n(k)$, leaving only a fourier transform which we define $ \mathcal{F} : f\rightarrow \frac{1}{\sqrt{2\pi}} \int_{-\infty}^{\infty}f(k)e^{\mathbb{i}\Lambda k}\mathrm{d}k$. To clean the formula up even further we may use the convolution theorem to deal with the powers of $L$, leaving only the fourier transform of the Bessel function $J_0(k)$, which is the arcsine distribution $\alpha(\lambda)$. The eigenvalue density of a kronecker sum of matrices is the convolution of the densities of those matrices.
...
\end{align}
The eigenvalue distributions determine the eigenbasis evolution  \eqref{eq:eigenbasis-vector-evolution} which reveals that the second order derivative leads to exponential relaxation of all basis vectors, except for the subset with vanishing eigenvalue $\lambda=0$. The corresponding zero eigenvectors $v_0$ are the fourier components of the steady state distribution, are the only components that remain as $t\rightarrow\infty$. The diffusive components $D_i$ determine the timescale of relaxation in each dimension.

\subsubsection{Turing Bifurcations}
The partial derivatives in the diffusion equation \eqref{eq:reaction-diffusion} can be converted into algebraic expression with the help of fourier transforms. Suppose we define the spatial-temporal fourier transform as
\begin{align}
...
\begin{equation}
	(\omega\mathbb{I}+ |k|^2  D)\mathcal{F}[u] = \mathcal{F}[\rates]
\end{equation}
...
\begin{equation}
	\left(\left.\jacobian\right|_{\steady} - |k|^2D - \omega\mathbb{I} \right)\mathcal{F}[u] = 0
\end{equation}
identifying $\omega$ and the eigenvalues to corresponding eigenvectors $\mathcal{F}[u]$. Using the determinant equation
\begin{equation}
	\left|\,\,\left.\jacobian\right|_{\steady} - |k|^2D - \omega\mathbb{I} \right| = 0
	\label{eq:dispersion-relation}
\end{equation}
\begin{Figure}
\includegraphics[width=0.5\linewidth]{turing-bifurcation}
\caption{A \emph{Turing bifurcation} occurs at condition $p=p^\star$ when the the dispersion relation crosses the imaginary axis $\Real[\omega(k)]>0$. A region of unstable modes pushes the dynamics away from homogenous state $\steady$ giving rise to patters}
\label{fig:turing-bifurcation}
\end{Figure}
We can solve for the dispersion $\omega(k)$
which yields the conditions for necessary for diffusion-driven instability \cite{Strogatz2000}. Figure \ref{fig:turing-bifurcation} illustrates a typical dispersion relation that indicates a \emph{Turing} bifurcation at $p=p^\star$. The unstable modes $k$ where $\Real[\omega(k)]>0$ prevent the system from reaching a homogenous steady state $\steady$ and result in patterns. What may seem surprising is that the homogenous steady state $\steady$ is stable in the absence of diffusion $k=0$.

Whether its through eigenvalue distributions \eqref{eq:laplacian-distribution} or through dispersion relations \eqref{eq:dispersion-relation} it is possible to get a handle on pattern formation by looking at when eigenvalues cross the imaginary axis. Progress has been made toward pattern analysis beyond linear stability \cite{Halatek2018}. In this setting we consider local equilibria in state space, the dynamics of which are determined by reactive and diffusive fluxes.
...
\begin{Figure}
	\includegraphics[width=0.8\linewidth]{reaction-diffusion-balance}
	\caption{The competition between reactive forces $\rates$ pulling towards fixed points $\steady$ and diffusive fluxes $\frac{\partial u}{\partial x}$ pulling towards the spatial average $\langle u(x,t) \rangle$ dictates local dynamics and hence the velocity of domain boundary}
...
\caption{Top: \textit{Hox} gene expression patterns in body segments of drosophila Bottom: Mutation where legs grow in-place of antenna \cite{Several2022HoxGene}}
\label{fig:morph}
\end{Figure}
During the development of any organism a hierarchy of self-organisation takes place that leads to the breaking of symmetry from a spherical cluster of undifferentiated cells to the formation of organ segments and limbs. This process is known as morphogenesis. Figure \ref{fig:morph}, taken from \cite{Several2022HoxGene}, shows an example in which \textit{Hox} gene expression patterns in the body segments of Drosophila can drastically affect its development.
...
\caption{Expression patterns of pair-rule \textit{Gap} genes in Drosophila embryo \cite{Hejnol2017LarvalLater}}
\label{fig:gap}
\end{Figure}
