...
Every cell belonging an organism has exactly the same genetic information inside it. In spite of this, a wide variety of cell populations such as neurons, muscle cells, immune cells and tissue specific cells interact in a highly regulated fashion. As an organism develops, how do cells which used to be identical differentiate into functionally distinct populations? How are boundaries between these populations maintained? Can these boundaries be controlled with genetic engineering? 
...
We assume rates $\alpha,\alpha^\prime$ of transcription and translation of \emph{LacI} into \emph{LacR} and \emph{LacZ} into $\beta$-Gal are constant. \emph{LacZ} is not transcribed when the operator region is blocked by \emph{LacR}, which we mark by an asterisk. The repressor can also be inactivated by the binding of two (allo)lactose molecules $\ce{C12H22O11}$.

\begin{align*}
	\ce{\bond{~}LacI\bond{~} &->[$\alpha$] \bond{~}LacI\bond{~} + LacR \\
	\bond{~}Operator\bond{~}LacZ\bond{~} &->[$\alpha^\prime$]
	\bond{~}Operator\bond{~}LacZ\bond{~} + $\beta$-Gal \\
	C12H22O11 &->[$\beta$-Gal][+H2O] C6H12O6 + C6H12O6 \\
	\bond{~}Operator\bond{~}LacZ\bond{~} + LacR &<=>[k] \bond{~}Operator^*\bond{~}LacZ\bond{~} \\
	LacR + 2C12H22O11 &<=>[k^\prime] LacR^*}
\end{align*}
Since all proteins degrade over some finite half-life $\mu$ we also have
\begin{align*}
	\ce{$\beta$-Gal &->[$\mu$] \emptyset \\
	LacR ->[$\mu$] \emptyset \quad&\quad LacR^* ->[$\mu$] \emptyset}
\end{align*}

The equilibrium dissociation constants $k,k'$ define the binding affinity if \emph{LacR} to the operator and its effector respectively. Let rate $\beta$ be the rate of hydrolysis of lactose by $\beta$-Gal. These rates are determined by molecular details, some of which can be modulated with genetic engineering. Changing the promoter sequences, for example, can modulate the rates $\alpha,\alpha'$. Thus it makes sense to collect these rates into our model parameter $\theta=(\alpha,\alpha',\beta,k,k')$. Let the concentration of the effector be the experimentally controlled condition $p=[\ce{C12H22O11}]$ and the remaining concentrations be collected in state vector $u$.
...
\begin{align}
\frac{d}{dt}\mathrm{LacR} =  \mathrm{LacR}^*- \frac{p^2}{2} k^{\prime} \mathrm{LacR} &+  \mathrm{LacZ}^*  - k \mathrm{LacR}\,\mathrm{LacZ} + \alpha -\mu\mathrm{LacR}\nonumber\\
\frac{d}{dt}\mathrm{LacR}^* =  -  &\mathrm{LacR}^* + \frac{p^{2}}{2} k^{\prime} \mathrm{LacR}-\mu\mathrm{LacR}^* \\\nonumber\\
\frac{d}{dt}\mathrm{LacZ} = & \mathrm{LacZ}^* - k \mathrm{LacR}\,\mathrm{LacZ}\nonumber\\
\frac{d}{dt}\mathrm{LacZ}^* =  - & \mathrm{LacZ}^* + k \mathrm{LacR}\,\mathrm{LacZ} \\\nonumber\\
\frac{d}{dt} \mathrm{Gal}= &\alpha^\prime \mathrm{LacZ} - \mu\mathrm{Gal} \nonumber\\
\frac{dp}{dt} = 2 \mathrm{LacR}^* &-p^{2} k^{\prime} \mathrm{LacR} - \beta p\,\mathrm{Gal} 
\end{align}
Since we are experimentally controlling the lactose concentration we can set $\frac{dp}{dt}=0$ and rearrange for the equilibrated concentration of deactivated repressor $\mathrm{LacR}^*$. Furthermore the total concentration of the $\mathrm{LacZ}$ gene is conserved which allows us to normalise by it
\begin{align}
	\mathrm{LacR}^* &= p \frac{ \beta \,\mathrm{Gal}+p k^{\prime} \mathrm{LacR}}{2}\\
	\mathrm{LacZ}^* &=1-\mathrm{LacZ}
\end{align}
This allows us to focus of equations for $\mathrm{LacR}$, $\mathrm{LacZ}$ and $\mathrm{Gal}$ because we can derive the other concentrations from them.

% Let us write down a system of differential equations that describes this regulatory network. We begin by writing down two equations that describe the changes to mRNA concentrations $r\in\Reals^N$ and protein concentrations $u\in\Reals^N$ in a typical transcription-translation process
% \begin{equation}
% 	\frac{dr}{dt}=H(u) - \mu r
% 	\qquad
% 	\frac{du}{dt}=F(r,u) - \gamma u
% \end{equation}
% where the vector function $H(u)$ quantifies the rate of transcription of DNA into mRNA in the presence of proteins $u$. The function $H(u)$ would encapsulate how proteins activate or inhibit the transcription of genes via mechanisms such as changes to chromatin conformation, interaction with transcription factors and binding regions of DNA. The function $F(r,u)$ quantifies the rate of translation of mRNA into proteins by ribosomes. Biological processes that modulate mRNA stability, RNA interference and other post-transcriptional regulatory mechanisms are encoded by this function.

% If we assume that mRNA dynamics equilibrate much faster than proteins, we can focus exclusively on protein concentrations $u$ but solving for the equilibrium concentrations of mRNA $r^*$
% \begin{equation}
% 	\frac{du}{dt}=F(r^*,u) - \gamma u
% 	\where r^* = \frac{H(u)}\mu
% \end{equation}
% Note that an important condition that allows us to move forward is the knowledge that transcription rates $H$ do not depend on mRNA concentrations $r$.
