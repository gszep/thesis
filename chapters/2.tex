...
\begin{Figure}
\includegraphics[scale=0.72]{fields}
...
\end{equation}
The Jacobian evaluated at fixed points $\left.\jacobian\right|_{u=\steady}$ can be expressed in terms of its eigenvectors $v_\lambda$ and eigenvalues $\lambda$ and therefore 
\begin{equation}
	\delta\steady(t)  \approx \sum_{\lambda}
	\alpha_{\lambda}v_{\lambda}e^{\lambda(t-t')}\quad\mathrm{where}\quad
	\lambda v_{\lambda} = \lambda\left.\jacobian\right|_{u=\steady} v\in\Reals^N
\end{equation}
\begin{equation*}
...
\end{equation*}
We can see that the sign of the real parts to eigenvalues $\Re\mathrm{e}\lambda$ determine the exponential growth or shrinkage of fixed point perturbations $\delta\steady(t)$ in the direction parallel to the corresponding eigenvector $v_{\lambda}$. The imaginary parts $\Im\mathrm{m}\lambda$ determine the frequency of oscillations around the fixed point. If at least one of the eigenvalues has a positive real part $\Re\mathrm{e}\lambda$ the field pushes trajectories away from the fixed point in direction $v_{\lambda}$ which means the fixed point is unstable.
