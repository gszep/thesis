...
We have seen that partial differential equations can be cast into high-dimensional ordinary differential equation form. This yields eigenvalue distributions (Section \ref{section:eigenvalue-distributions}) that dictate the stability of different spatial modes. Naively applying the method outlined in Chapter \ref{chapter:inference} can be used to design snaking \cite{Uecker2021ContinuationExperiments}, and hence pattern-forming regimes, especially since \texttt{BifurcationKit.jl} \cite{Veltz2020BifurcationKit.jl} was originally designed to deal with high-dimensional problems. The gradient calculation --- which would scale like $(NK)^2$ where $N$ is the number of states and $K\gg 1$ the number of spatial discretisations --- would be the limiting factor, and would have to be optimised, possibly using sparse tri-diagonal matrix methods. Calculations can be further sped up


The dependence of eigenvalues on modes can also be determined via the dispersion relation