...
%This chapter outlines the methods applied in all work herein. Chapter (\ref{GO}) relies on methods covered in sections (\ref{chapter:MD}) and (\ref{chapter:onetep}), chapter(\ref{ProteinCorona}) relies only on section (\ref{chapter:MD}) and chapter (\ref{Hemocyanin}) relies on section (\ref{chapter:onetep}). Where applicable, any use of further methods have been outlined in-depth within the content of the respective chapters.
...
The Hohenberg-Kohn theorem starts by stating that the external potential ($V(\mathbf{r})$) is uniquely determined by the electron density ($n(\mathbf{r})$), and this is proven with a simple \textit{reductio ad absurdum} argument. Consider a many-electron Hamiltonian of the form ($H = T + U + V$) and a a different Hamiltonian ($H' = T + U + V'$), having wave functions ($\Psi$) and ($\Psi '$) respectively and where ($V - V' \neq constant$). The electron density is defined by (Eq.~\ref{density}), we assume that the electron density is equal for $V$ and $V'$. 
%
...
\end{equation}
%
%where the KS orbitals are orthonormal to one another
%\begin{equation} \label{ortho}
%\int \psi_i^*(\mathbf{r}) \psi_j(\mathbf{r}) d \mathbf{r} = \delta_{ij}
%\end{equation}
%
%The variation of the energy $E$ must vanish under an arbitrary variation of $\psi_i(\mathbf{r})$, given the orthonormality in (\ref{ortho}):
%
%\begin{equation}
%E' = E - \sum_{ij} \lambda_{ij} \left(\int \psi_i^*(\mathbf{r}) \psi_j(\mathbf{r}) d \mathbf{r} - \delta_{ij} \right)
%\end{equation}
%
